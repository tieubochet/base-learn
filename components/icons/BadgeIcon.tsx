...
import { useDeployContract, useWaitForTransactionReceipt, useAccount, useWriteContract } from 'wagmi';
import { baseSepolia } from 'viem/chains';
import { ContractInfo, BADGE_CONTRACT } from '../constants/contract';
...
export const ContractCard: React.FC<ContractCardProps> = ({ contract }) => {
  ...
  // FIX: Destructure address from useAccount to pass to mintContract call.
  ...
  const handleMint = () => {
    if (!contract.badgeUri) {
        alert("This contract doesn't have a badge URI configured.");
        return;
    }
    // FIX: Add a guard to ensure wallet is connected and address/chain are available.
    if (!address || !chain) {
        alert("Wallet must be connected to mint a badge.");
        return;
    }
    resetMint();
    // FIX: Pass account and chainId to the mintContract call to satisfy the wagmi hook's required parameters.
    mintContract({
        address: BADGE_CONTRACT.address,
        abi: BADGE_CONTRACT.abi,
        functionName: 'mint',
        args: [contract.badgeUri],
        account: address,
        chainId: chain.id,
    });
  };
    ...
    <div className="bg-base-surface p-6 rounded-lg shadow-lg border border-slate-700 flex flex-col justify-between space-y-4">
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button
                    onClick={handleMint}
                    disabled={!contract.hasBadge || !contract.badgeUri || !isConfirmed || isMinting || isMintConfirming}
                ...
                >
                    {isMinting || isMintConfirming ? (
                        <>
                            ...
                            <span>{isMinting ? 'Confirm...' : 'Minting...'}</span>
                        </>
                    ) : (
                       contract.hasBadge ? (
                            <>
            ...
            <div className="mt-4 space-y-2">
                ...
                {mintHash && (
                    <StatusCard type="info" link={`${baseSepolia.blockExplorers.default.url}/tx/${mintHash}`}>
                        Mint transaction submitted.
                    </StatusCard>
                )}
                {isMintConfirmed && (
                    <StatusCard type="success" link={`${baseSepolia.blockExplorers.default.url}/tx/${mintHash}`}>
                        Badge minted successfully!
                    </StatusCard>
                )}
                {mintError && (
                    <StatusCard type="error">
                        <p className="truncate">Mint failed: {mintError.message}</p>
                    </StatusCard>
                )}
            </div>
        </div>
